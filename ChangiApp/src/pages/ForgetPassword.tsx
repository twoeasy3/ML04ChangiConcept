...
const ProgressBarSection: React.FC<ProgressBarSectionProps> = ({completed,active,isFirst}) => {
...
const ProgressBar: React.FC<ProgressBarProps> = ({currentStep}) => {
    ...
    return(
        <div className = "PROGRESSBARPARENT justify-between flex w-11/12">
            {steps.map((step,index)=>(
                <React.Fragment key={index}>
                    <ProgressBarSection completed = {index < currentStep} active = {index === currentStep} isFirst = {index === 0}></ProgressBarSection>
                </React.Fragment>
            ))}
        </div>
    );
